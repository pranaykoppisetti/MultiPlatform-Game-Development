...
public class controller : MonoBehaviour{
    ...
    private void calculateEnginePower(){
        ...
        if(!engineLerp){
            engineRPM = Mathf.Lerp(engineRPM,10000f + Mathf.Abs(wheelsRPM) *  finalDrive *  (gears[gearNum]) , (EngineSmoothTime * 10) * Time.deltaTime);
    ...
    private void shifter(){
        ...
        if(engineRPM > maxRPM  && gearNum < gears.Length-1 && !reverse && Time.time >= gearChangeRate  && KPH >35){
            gearNum ++;
            audio.DownShift();
    ...
    public void lerpEngine(){
        if(engineLerp){
            ...
            engineLerp = engineRPM <= engineLerpValue + 500 ? false : true;
        }
    }   
